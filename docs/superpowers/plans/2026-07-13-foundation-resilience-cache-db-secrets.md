...
**Architecture:** One Deno workspace at `backend/` (sibling to `src/` and `src-tauri/`, kept separate from the npm/Vite project until the Plan 6 cutover), with one module per subdirectory (`resilience/`, `cache/`, `db/`, `secrets/`). As actually implemented, each module exposes its public API directly from its implementation file(s) (e.g. `resilience/circuit_breaker.ts`, `cache/memory.ts`) rather than through a separate `mod.ts` barrel — later plans import directly from these files. Every module is pure TypeScript with zero dependency on frontend code or Tauri, and zero cross-module imports (verified in the Task 10 whole-branch review: `cache/sqlite.ts`'s `SqliteCache` takes an already-open `DatabaseSync` via constructor injection rather than importing `db/connection.ts` itself, so even the one cross-module dependency this plan anticipated didn't end up needed). Tests run via `deno test`.
...
- **Invocation directory (discovered during Task 6):** this repo has a root `package.json` (the existing npm/Tauri/Vite app). Deno's `noImplicitAny`/`strict` compiler options from `backend/deno.json` apply correctly regardless of the caller's working directory. But **any file importing `node:sqlite`** (Task 6 onward) also needs `@types/node`, and Deno's npm-compat resolution for that only finds `backend/node_modules/@types/node` when the working directory is `backend/` itself — not when invoked from the repo/worktree root. So: run every `deno test`/`deno lint`/`deno task` command from inside `backend/` (`cd backend && deno test <path-without-a-"backend/"-prefix>`, or `cd backend && deno task test`), not from the repo root. `backend/package.json` (added in Task 6) declares the `@types/node` devDependency scoped to this workspace — the shared root `package.json` is never touched.
- **One-time setup for a fresh clone or CI (discovered during Task 6 review):** `backend/deno.json` sets `"nodeModulesDir": "manual"`, so `backend/node_modules/` (gitignored) is never auto-populated. Before running any test in `backend/` for the first time in a fresh checkout, run `cd backend && deno install` once (or `npm install` from `backend/`, since `backend/package.json` declares the same `@types/node` devDependency) to provision it. Without this step, `deno test`/`deno task test` fails with "Could not find `@types/node` in a node_modules folder."
...
- Every module (`resilience/`, `cache/`, `db/`, `secrets/`) exposes its public API directly from its implementation file(s), with zero cross-module imports in production code — `cache/sqlite.ts`'s `SqliteCache` receives an already-open `DatabaseSync` via constructor injection rather than importing `db/connection.ts` directly.
...
- **Windows concurrency hazard**: `windowsSet`/`windowsGet` pass the secret value to `powershell` via process-global `Deno.env.set(...)`/`.delete(...)`, which races if `setSecret`/`getSecret` are ever called concurrently on Windows (e.g. provisioning multiple API keys in parallel) — one call could read another's in-flight value. **Explicit precondition for whichever later plan first calls `setSecret` to provision more than one key** (per the whole-branch review, the realistic trigger is a settings screen provisioning the 9 `.env.example` API keys via `Promise.all(...)`): before that caller does concurrent Windows secret access, either serialize calls into this module (a simple queue/mutex) or find a non-global way to pass the value to `powershell`. Do not add concurrent `setSecret`/`getSecret` calls on Windows until this is addressed.
