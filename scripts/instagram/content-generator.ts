...
export const COMPOSITIONS: Record<string, CompositionConfig> = {
  ...
  // Feed post compositions (1080×1080 stills)
  'feed-tip': {
    id: 'feed-tip',
...
const THEMES: Record<string, ContentTheme> = {
  'vibe-investing': {
    hook: [
      ...
      'Hot take: most portfolio templates are designed for the person who made them, not you.',
      ...
      'The difference between gambling and investing is a thesis. Here\'s how to build yours.',
  ...
  'privacy-first': {
    hook: [
      ...
      'Fun fact: most fintech apps know more about your finances than your accountant does.',
  ...
  'backtest': {
    hook: [
      ...
      'Conviction without evidence is just hope. Here\'s how to get the evidence.',
      'Before you risk a single dollar, FlowFolio will show you what would have happened if you risked a million.',
      'Hot take: if you haven\'t backtested your strategy, you don\'t have a strategy \u2014 you have a guess.',
  ...
  'quant-metrics': {
    hook: [
      ...
      'Sharpe ratio. Sortino ratio. Information ratio. You need to know all three. Here\'s why.',
      ...
      'PE ratio alone tells you almost nothing. Here\'s what to look at instead.',
    ...
    cta: [
      ...
      'Level up your analysis. Follow @flowfolio for quant insights.',
    ],
    hashtags: [
  ...
  'education': {
    hook: [
      'Swipe to learn the one concept that changed how I think about portfolio risk.',
      'I wish someone had explained factor investing this clearly when I started.',
      'This 60-second breakdown is worth more than most investing courses.',
      'Bookmark this. You\'ll come back to it more than you think.',
      'The concepts in this carousel separate informed investors from the crowd.',
      'No jargon. No gatekeeping. Just the investing knowledge you actually need.',
      'I spent 200 hours learning this. You can learn it in 8 slides.',
      'Stop scrolling. This might be the most useful investing post you see today.',
    ],
    body: [
  ...
  'market-data': {
    ...
    body: [
      ...
      'Here\'s the data stack in FlowFolio:\n\nTier 1: Alpaca (unlimited, free)\nTier 2: Finnhub, FMP, Tiingo, Twelve Data\nTier 3: Polygon, Alpha Vantage\nFallback: Yahoo Finance (no key needed)\n\nIf Tier 1 is slow, Tier 2 picks up. If everything fails, Yahoo Finance catches the request. Your data pipeline has no single point of failure.',
  ...
  'founder-story': {
    hook: [
      ...
      'Here\'s what happens when an engineer gets frustrated with fintech: open-source investing software.',
    ],
    body: [
...
/**
 * Generate expert-quality captions with varied structures.
...
export function generateCaption(theme: string, seed: number): { caption: string; hashtags: string } {
  ...
  switch (format) {
    ...
    case 2: {
      // Story-led: personal angle + body + takeaway + CTA
      const altHook = pickSecond(t.hook, seed >> 2);
      caption = `${hook}\n\n${body}\n\nThe takeaway: ${altHook.toLowerCase().replace(/\.$/, '')}\n\n${cta}`;
      break;
    }
...
function renderStill(comp: CompositionConfig, seed: number): string {
  ...
  console.log(`🖼️  Rendering ${comp.displayName} (seed: ${seed})...`);
...
function renderCarousel(comp: CompositionConfig, seed: number): string {
  ...
  console.log(`🖼️  Rendering ${comp.displayName} (seed: ${seed}, ${slideCount} slides)...`);
...
export function generateContentPlan(seed: number, mix: ContentMix = 'ig-only'): {
  ...
  // Carousels always use the education theme
  const theme = composition === 'carousel' ? 'education' : pickTheme(seed >> 4);
