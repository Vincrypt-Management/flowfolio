...
impl FreeDataProviders {
    ...
    pub async fn fetch_google_finance(&self, symbol: &str) -> Result<MarketDataResult, String> {
        let url = format!("https://www.google.com/finance/quote/{}:NASDAQ", symbol);
        ...
        if !response.status().is_success() {
            ...
            let response_nyse = crate::HTTP_CLIENT
                .get(&url_nyse)
                .send()
                .await
                .map_err(|e| format!("Google Finance request failed: {}", e))?;
    ...
    pub async fn fetch_stooq_historical(
        &self,
        symbol: &str,
    ...
    pub async fn fetch_nasdaq_historical(
        &self,
        symbol: &str,
    ) -> Result<Vec<HistoricalPrice>, String> {
        let url = format!(
        ...
        let historical: Vec<HistoricalPrice> = rows
            .iter()
            .filter_map(|row| {
    ...
    pub async fn fetch_investingcom(&self, symbol: &str) -> Result<StockQuote, String> {
        ...
        let stock = quotes
            .iter()
            .find(|q| q.get("symbol").and_then(|s| s.as_str()) == Some(symbol))
    ...
    pub async fn fetch_sec_company_info(
        &self,
        symbol: &str,
    ) -> Result<CompanyFundamentals, String> {
        ...
        // Get company facts
        let facts_url = format!("https://data.sec.gov/api/xbrl/companyfacts/CIK{}.json", cik);
        ...
        let facts: Value = facts_response
            .json()
            .await
            .map_err(|e| format!("Parse error: {}", e))?;
    ...
    pub async fn fetch_yahoo_quote_summary(
        &self,
        symbol: &str,
    ) -> Result<CompanyFundamentals, String> {
        let url = format!(
        ...
        let get_raw =
            |obj: Option<&Value>, key: &str| -> Option<f64> { obj?.get(key)?.get("raw")?.as_f64() };
    ...
    pub async fn search_yahoo(&self, query: &str) -> Result<Vec<(String, String)>, String> {
        ...
        let results: Vec<(String, String)> = quotes
            .iter()
            .filter_map(|q| {
