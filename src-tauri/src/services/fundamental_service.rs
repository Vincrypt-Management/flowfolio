...
impl FundamentalDataService {
    ...
    async fn fetch_from_yahoo(&self, symbol: &str) -> Result<FundamentalMetrics, String> {
        ...
        Ok(FundamentalMetrics {
            ...
            sector: Self::extract_string(price_data, &["sector"])
                .unwrap_or_else(|| "Unknown".to_string()),
            industry: Self::extract_string(price_data, &["industry"])
                .unwrap_or_else(|| "Unknown".to_string()),
            ...
            price_to_sales: Self::extract_raw_number(
                summary_detail,
    ...
    pub async fn get_batch_fundamentals(
        &self,
        symbols: Vec<String>,
    ) -> HashMap<String, FundamentalMetrics> {
...
mod tests {
    ...
    fn test_parse_av_number_valid_key_returns_parsed_f64() {
        ...
        assert!((result - 25.5).abs() < 0.001, "expected 25.5, got {result}");
    }
