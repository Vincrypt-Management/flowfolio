...
const FEATURES = [
  {
    label: 'Dashboard',
    desc: 'Portfolio overview with sector donut & top movers',
    icon: '📊',
    color: colors.primary,
  },
  {
    label: 'Risk Dashboard',
    desc: 'Composite score gauge, VaR & correlation heatmap',
    icon: '🛡️',
    color: colors.rose,
  },
  {
    label: 'Comparison Mode',
    desc: 'Side-by-side ticker charts & quant metrics',
    icon: '🔀',
    color: colors.accent,
  },
  {
    label: 'Price Alerts',
    desc: 'Configurable thresholds with auto-monitoring',
    icon: '🔔',
    color: colors.amber,
  },
  {
    label: 'News & Sentiment',
    desc: 'Real-time market news with bullish/bearish scoring',
    icon: '📰',
    color: colors.cyan,
  },
  {
    label: 'Rebalance Scheduler',
    desc: 'Timeline visualization & overdue detection',
    icon: '📅',
    color: colors.blue,
  },
  {
    label: 'Watchlist',
    desc: 'Track symbols across universes with live prices',
    icon: '👁️',
    color: colors.primaryBright,
  },
  {
    label: 'Credits Dashboard',
    desc: 'Balance ring, tier info & usage meters',
    icon: '💳',
...
const STATS = [
  { value: '8', label: 'New Components' },
  { value: '6', label: 'Bug Fixes' },
  { value: '0', label: 'Rust Warnings' },
  { value: '296', label: 'Tests Passing' },
];
    ...
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ opacity: 0.025 }}>
        <svg width="100%" height="100%">
          <defs>
            <pattern id="relDot" x="0" y="0" width="44" height="44" patternUnits="userSpaceOnUse">
              <circle cx="22" cy="22" r="0.5" fill="#ffffff" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#relDot)" />
        </svg>
      </AbsoluteFill>
      <div
        style={{
...
const HookScene: React.FC = () => {
  ...
  const countOp = interpolate(frame, [10, 40], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.out(Easing.cubic) });
  const countScale = interpolate(frame, [10, 50], [0.6, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.out(Easing.back(2)) });
  const line1Op = interpolate(frame, [50, 80], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.out(Easing.cubic) });
  const line1Y = interpolate(frame, [50, 80], [12, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.out(Easing.cubic) });
  const line2Op = interpolate(frame, [80, 110], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.out(Easing.cubic) });
  const line2Y = interpolate(frame, [80, 110], [10, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.out(Easing.cubic) });
  const fadeOut = interpolate(frame, [230, 270], [1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });

  return (
    <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center', opacity: fadeOut }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, padding: '0 48px' }}>
        <div style={{ fontSize: 120, fontWeight: 800, color: colors.primary, fontFamily: fonts.sans, opacity: countOp, transform: `scale(${countScale})`, letterSpacing: '-4px' }}>
          8
        </div>
        <div style={{ fontSize: 42, fontWeight: 700, color: colors.text, fontFamily: fonts.sans, letterSpacing: '-0.04em', textAlign: 'center', lineHeight: 1.15, opacity: line1Op, transform: `translateY(${line1Y}px)` }}>
          new superpowers
        </div>
        <div style={{ fontSize: 42, fontWeight: 700, color: colors.textMuted, fontFamily: fonts.sans, letterSpacing: '-0.04em', textAlign: 'center', lineHeight: 1.15, opacity: line2Op, transform: `translateY(${line2Y}px)` }}>
          for your portfolio
        </div>
      </div>
    </AbsoluteFill>
    ...
    <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center', opacity: fadeOut }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24 }}>
        ...
        <div style={{ opacity: badgeOp, transform: `translateY(${badgeY}px)`, padding: '10px 28px', borderRadius: radius.xl, background: `linear-gradient(135deg, ${colors.primary}, ${colors.accent})` }}>
          <span style={{ fontSize: 22, fontWeight: 700, color: colors.bg, fontFamily: fonts.mono }}>v0.2.2</span>
        </div>
      </div>
    </AbsoluteFill>
  );
...
const FeatureListScene: React.FC<{ features: typeof FEATURES; batch: number }> = ({ features, batch }) => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center', opacity: fadeOut }}>
      <div style={{ fontSize: 14, fontWeight: 600, color: colors.primary, fontFamily: fonts.mono, letterSpacing: '0.08em', textTransform: 'uppercase' as const, marginBottom: 28, opacity: interpolate(frame, [10, 40], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }) }}>
        {batch === 0 ? 'New in v0.2.2' : 'And more...'}
      </div>
    ...
    <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center' }}>
      ...
      <div style={{ marginTop: 40, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, opacity: ctaOp, transform: `translateY(${ctaY}px)` }}>
        <div style={{ padding: '16px 52px', borderRadius: radius.lg, background: colors.primary, boxShadow: '0 8px 24px rgba(0, 229, 153, 0.2)' }}>
          <span style={{ fontSize: 20, fontWeight: 600, color: colors.bg, fontFamily: fonts.sans }}>Your Portfolio. Your Rules.</span>
        </div>
        <span style={{ fontSize: 14, fontFamily: fonts.mono, color: colors.primary, opacity: linkOp }}>Privacy-first · AI-powered · Free</span>
      </div>
...
export const FlowFolioRelease022: React.FC<ReleaseReelProps> = ({ seed }) => {
  ...
  return (
    <VideoSeedContext.Provider value={rng}>
      <AbsoluteFill style={{ backgroundColor: colors.bg, fontFamily: fonts.sans, overflow: 'hidden' }}>
        ...
        {/* Features batch 1: Dashboard, Risk, Compare, Alerts (9.3s-16s) */}
        <Sequence from={560} durationInFrames={400}>
          <FeatureListScene features={FEATURES.slice(0, 4)} batch={0} />
        </Sequence>

        {/* Features batch 2: News, Scheduler, Watchlist, Credits (15.7s-22.3s) */}
        <Sequence from={940} durationInFrames={400}>
          <FeatureListScene features={FEATURES.slice(4, 8)} batch={1} />
        </Sequence>
